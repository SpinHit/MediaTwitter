...
export default function MediaApp(){
    ...
    const [dataProfile, setData2] = useState([])
     ...
     useEffect(() => {
        (async () => {
            const data1 = await axios.get('http://127.0.0.1:8000/api/tweet/Media')
            const data2 = await axios.get('http://127.0.0.1:8000/api/profile/Media')
            setData(data1.data);
            setData2(data2.data);
        })()
    }, [])
  return (
    <div className="tweetsZone">
       {data.slice(0,5).map(item =>(
         ...
         <img src={dataProfile.profile_image_url} alt="profile Mediavenir"/>
